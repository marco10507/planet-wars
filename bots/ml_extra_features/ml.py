...
class Bot:
    ...
    def value(self, state, alpha=float('-inf'), beta=float('inf'), depth = 0):
        ...
        for move in moves:
            ...
            value, _ = self.value(state.next(move), alpha, beta, depth + 1)
...
def features(state):
    ...
    for planet in state.planets():
        if state.owner(planet) == 1:
            p1_garrisons += state.garrison(planet)
        elif state.owner(planet) == 2:
    ...
    # How many conquered planets has the p1?
    p1_planets = 0.0
    # How many conquered planets has the p2?
    p2_planets = 0.0
